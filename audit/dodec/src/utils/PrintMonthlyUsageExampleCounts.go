...
// PrintMonthlyUsageExampleCounts prints a nicely-formatted series of tables with counts of new usage
...
func PrintMonthlyUsageExampleCounts(productSubProductCounter types.NewAppliedUsageExampleCounterByProductSubProduct, monthForReporting time.Month) {
	...
	for collectionName, pagesToPrintInCollection := range productSubProductCounter.PagesInCollections {
		...
		printSeparator(pageIdTableColumnWidths...)
		fmt.Printf("Total new usage example counts in collection %s: %d\n", collectionName, collectionCount)
	}
	...
	appliedUsageExampleCountsColumnWidths := []int{30, 18, 25, 18}
