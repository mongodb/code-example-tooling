...
interface ConversionOptions {
  inputDir: string;
  outputDir: string;
  handleIncludes: boolean;
  handleSubstitutions: boolean;
  handleRefs: boolean;
}

interface ApiConversionOptions {
...
async function convertRealmDocs(options: ConversionOptions): Promise<number> {
  ...
  for (const file of files) {
    try {
      ...
      const parsedContent = await parseSnootyContent(content, {
        filePath: file,
        basePath: inputDir,
      ...
      const outputPath = path.join(
        outputDir,
        relativePath.replace(/\.(txt|rst)$/, '.md')
      );
...
async function convertRealmDocsFromApi(options: ApiConversionOptions): Promise<number> {
  ...
  const pages = await fetchSnootyProject({ project, branch, baseUrl });
  ...
  for (const page of pages) {
    try {
      ...
      const md = astToMarkdown(page.ast, {
        substitutions,
        onWarn: (message, ctx) => {
...
export { convertRealmDocs, convertRealmDocsFromApi };
...
function copyDirRecursive(srcDir: string, destDir: string) {
  ...
  // Fallback
  const entries = fs.readdirSync(srcDir, { withFileTypes: true } as any);
  for (const entry of entries as any[]) {
    const srcPath = path.join(srcDir, (entry as any).name);
    const dstPath = path.join(destDir, (entry as any).name);
...
function rewriteImagePaths(markdown: string, outPath: string, outputRoot: string): string {
  try {
